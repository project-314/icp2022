...
if ($request_data->action == "insert") {
    ...
    $member_id = (int)$request_data->member_id;
    $query = "SELECT plc.plan_career_id, plc.start_date,
                     plc.career_id, car.career_name
                FROM plan_career    AS plc

                JOIN member         AS mem ON plc.member_id         = mem.member_id
                ...
                GROUP BY plc.plan_career_id
                ORDER BY plc.plan_career_id ASC
             ;";
    $statement = $connect->prepare($query);
    $statement->execute();
    while ($row = $statement->fetch(PDO::FETCH_ASSOC)) {
