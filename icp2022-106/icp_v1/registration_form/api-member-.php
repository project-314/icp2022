...
if ($request_data->action == "insert") {
    ...
    $member_id  = (int) $request_data->member_id;
    $query = "SELECT mem.member_id, mem.full_name, mem.email, mem.password, mem.status
              FROM member AS mem
...
else if ($request_data->action == "getEmail") {
    $email = "'".$request_data->email."'";
    $query = "SELECT *
              FROM member
              WHERE email = $email
...
else if ($request_data->action == "checkUser") {
    $email = "'".$request_data->email."'";
    $query = "SELECT *
              FROM member
              WHERE email= $email
    ...
    $query = "SELECT member_id,full_name,status
              FROM member
    ...
    $query = "SELECT mem.member_id, mem.full_name, mem.email, mem.password, mem.status
              FROM member
              WHERE mem.member_id = $request_data->id";
    $statement = $connect->prepare($query);
    $statement->execute();
    while ($row = $statement->fetch(PDO::FETCH_ASSOC)) {
